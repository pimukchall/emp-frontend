              ...
              <v-col cols="12" sm="6">
                ...
                >
                  ...
                  <v-date-picker v-model="date" no-title scrollable>
                    ...
                    <v-btn text color="primary" @click="$refs.menu.save(date)">ยืนยัน</v-btn>
                  </v-date-picker>
                </v-menu>
              </v-col>
              </v-row>
...
export default {
  ...
  methods: {
    ...
    async UpdateData() {
      try {
        const req = await this.$store.dispatch('api/customer/putCustomers', this.data)
    ...
    recordLogUpdate(id) {
      const log = {
        ...
        action: 'อัพเดทข้อมูล',
        description: this.$auth.user.email + ' ' + `อัพเดทข้อมูลลูกค้า ID: ${id}` + ' ' + this.data.fname + ' ' + 'เวลา: ' + moment(new Date()).format('HH:mm:ss'),
