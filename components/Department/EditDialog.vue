<template>
  ...
  export default {
    ...
    methods: {
      ...
      async UpdateData() {
        try {
          const req = await this.$store.dispatch('api/department/putDepartments', this.data);
      ...
      const log = {
        ...
        action: 'อัพเดทข้อมูล',
        description: this.$auth.user.email + ' ' + 'อัพเดทข้อมูลแผนก' + ' ' + id + ' ' + 'เวลา' + ' ' + moment(new Date()).format('HH:mm:ss'),
